...
### Assign
**Class:** `System.Activities.Statements.Assign` (non-generic activity tag in Studio-emitted mature projects) / generic `Assign`1` only when the **whole project** already uses that form.
**XAML prefix:** standard activities namespace; no `ui:` prefix.

**Canonical Studio-readable snippet** (match this in mature projects — from real COE style):
```xml
...
**Notes:** **Default to the non-generic activity tag** with types only on `OutArgument`/`InArgument` and multi-line `Assign.To` / `Assign.Value`. That is what Studio emits and what users accept as not "looking odd." Prefer Assign-heavy logic over `Invoke Code`. One Assign per target. `DisplayName="Assign - <specific target/result>"`.

**Do not ship (janky in Designer even when XML-valid):**
```xml
<!-- collapsed To/Value -->
<Assign.To><OutArgument x:TypeArguments="x:String">[strX]</OutArgument></Assign.To>
<!-- generic type on Assign tag mixed into mature projects -->
<Assign x:TypeArguments="x:Object" DisplayName="Assign - Runtime flag">...</Assign>
```

**Studio-readable serialization:** Full wrong/right matrix, conversion steps, and greps: **[xaml/assign-studio-style.md](xaml/assign-studio-style.md)** (read before writing Assigns). Also [xaml/xaml-edit-fast-path.md](xaml/xaml-edit-fast-path.md).
