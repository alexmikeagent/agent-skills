...
## VB Expressions: Regex Binding and Multiline Fluent Chains

XML well-formedness does not prove that Studio can compile embedded VB expressions.

- `Regex.IsMatch(...)` requires `System.Text.RegularExpressions` in `TextExpression.NamespacesForImplementation`. An `<AssemblyReference>System.Text.RegularExpressions</AssemblyReference>` by itself is not a VB import. Prefer the fully qualified `System.Text.RegularExpressions.Regex` form when repairing a small or legacy workflow. `BC30451` plus a Data Manager suggestion usually means name resolution failed; it does not mean a `Regex` variable should exist.
- Do not format LINQ as `source.` followed by `.Where`, `.Select`, `.ToList`, and similar calls on later physical lines. UiPath's VB compiler can surface `BC30198: ')' expected`. Keep the complete fluent chain on one line or create intermediate typed variables with expanded Assign activities.
- Run `scripts/check_vb_xaml_expressions.py` on touched workflows in addition to `xmllint`.
...
## Assign Activities Look Odd / Janky in Studio

XML-valid Assigns often still render poorly in Designer. Real user feedback: "I am seeing them in studio but they look odd" → fix by matching a local example workflow (`brCheckComponentSumEvidence`) and expanding every Assign.

**Root causes (both common in agent output):**

1. Collapsed one-liners: `<Assign.To><OutArgument ...>...</OutArgument></Assign.To>`
2. Generic type on the activity tag: `<Assign x:TypeArguments="x:Object" ...>` mixed into projects whose Studio-emitted Assigns omit it

**Required shape:** multi-line `Assign.To` / `Assign.Value`, types only on `OutArgument`/`InArgument`, `DisplayName="Assign - <specific>"`, bracket `[expr]` when that is the local style.

**Full wrong/right matrix + greps:** [assign-studio-style.md](assign-studio-style.md).

```bash
rg -n '<Assign\.To><|<Assign\.Value><|<Assign[^>]*x:TypeArguments' <touched.xaml>
# expect zero matches in mature Studio-style projects
```

## Studio Reserialization (agent-authored XAML that Studio "fixes")

XML-valid workflows can still force Windows Studio to mass-reserialize or auto-repair the project. Prevent that by matching Studio contracts up front:
...
| Bare container bodies / compact indentation | Reserializes `TryCatch` / `If` / Sequences and ViewState sizes | Always wrap bodies in `<Sequence>`; preserve ViewState on untouched nodes |
| Compact one-line Assigns / mixed Assign styles | Painful Designer review (may not auto-expand) | Expanded `Assign.To` / `Assign.Value`; match local typing style |
| Duplicate or missing `WorkflowViewState.IdRef` | Unpredictable designer state | Unique IdRef per activity within the file |
| Mixed LF/CRLF after patching | Noisy diffs / Studio churn | Match the file's existing line endings |

**Local scanners must normalize both `/` and `\\`** when resolving `InvokeWorkflowFile` targets — e.g. `Framework/RetryCurrentTransaction.xaml` is valid even if other files use backslashes.

Full operational checklist: [xaml-edit-fast-path.md](xaml-edit-fast-path.md).
...
- **Path scanners**: When checking that targets exist, normalize `/` and `\\`. Do not flag cross-slash relative paths as missing.
