...
- Match the file's line endings and local Assign/Log serialization style ([xaml-edit-fast-path.md](xaml-edit-fast-path.md))
- **Assigns:** always multi-line Studio form — never collapsed To/Value or mixed generic tags ([assign-studio-style.md](assign-studio-style.md))
- Give new activities unique `sap2010:WorkflowViewState.IdRef` values; do not strip ViewState from untouched nodes

### Validate After Every Change
**Windows signed-in UiPath host:** Run `uip rpa validate` after every XAML modification; finish with project `build`. Do not batch multiple edits without validation.
**Mac host (default for this skill):** Do not run blocked `uip rpa validate`/`build`. Use the static gate in [xaml-edit-fast-path.md §7](xaml-edit-fast-path.md) and state that Windows compile validation remains outstanding.

### Entry-point arguments need sidecars
When changing arguments on `Main.xaml` / active entry points, also update `Main.xaml.json` (or `<Entry>.xaml.json`) and `entry-points.json`. XAML-only argument edits are a primary cause of Studio auto-repair diffs.
