...
**CRLF terminal-text rule.** UiPath terminal captures commonly contain `\r\n`. In .NET multiline mode, a row pattern such as `(?m)^(?&lt;row&gt;[^\r\n]+)$` can fail because `$` is reached after the content but before `\n`, while the intervening `\r` is excluded from the group. Match the carriage return explicitly outside the captured value: `(?m)^(?&lt;row&gt;[^\r\n]+)\r?$`, or normalize line endings before parsing. For paired fixed-width terminal rows, bind them with `\r?\n`, keep the code and amount rows from the same extracted grid, accept leading-decimal values such as `.00`, and add a CRLF regression fixture from a sanitized screen shape.
...
**VB `List(Of T).Count` predicate trap.** `List(Of T).Count` is a read-only property, not the LINQ predicate overload. In a UiPath VB expression, `myList.Count(Function(x) ...)` can bind to the property and fail with `BC32016: Count has no parameters`. Use `myList.Where(Function(x) ...).Count` for a filtered count (with `System.Linq` imported), or call `Enumerable.Count(myList, Function(x) ...)` explicitly. Keep a compile-oriented expression check for any new collection predicate.
