...
they do not specify a location, default to `~/.agents/skills` so the canonical
repository and runtime symlink stay aligned.
...
```bash
scripts/init_skill.py my-skill --path "$HOME/.agents/skills"
scripts/init_skill.py my-skill --path "$HOME/.agents/skills" --resources scripts,references
